<x-app-layout>
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <h2 class="text-xl font-bold mb-4">Edit Store</h2>
            ...
            <form action="{{ route('stores.update', $store->id) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="name" class="w-full p-2 border rounded" value="{{ old('name', $store->name) }}" required>
                    </div>

                    <div>
                        <label for="address" class="block font-medium">Address</label>
                        <input type="text" id="address" name="address" class="w-full p-2 border rounded" placeholder="Enter store address" value="{{ old('address', $store->address) }}" required>
                    ...
                    <div>
                        ...
                        <input type="text" name="contact" class="w-full p-2 border rounded" value="{{ old('contact', $store->contact) }}" required>
                    </div>

                    <div>
                        <label for="latitude" class="block font-medium">Latitude</label>
                        <input type="text" id="latitude" name="latitude" class="w-full p-2 border rounded bg-gray-100" value="{{ old('latitude', $store->latitude) }}" readonly required>
                    </div>

                    <div>
                        <label for="longitude" class="block font-medium">Longitude</label>
                        <input type="text" id="longitude" name="longitude" class="w-full p-2 border rounded bg-gray-100" value="{{ old('longitude', $store->longitude) }}" readonly required>
                    </div>

                    <div>
                        <label for="opening_time" class="block font-medium">Opening Time</label>
                        <input type="time" id="opening_time" name="opening_time" class="w-full p-2 border rounded" value="{{ old('opening_time', $store->opening_time) }}" required>
                    </div>

                    <div>
                        <label for="closing_time" class="block font-medium">Closing Time</label>
                        <input type="time" id="closing_time" name="closing_time" class="w-full p-2 border rounded" value="{{ old('closing_time', $store->closing_time) }}" required>
                    </div>
                </div>
